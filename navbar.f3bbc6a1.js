...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"assets/A thousand word speller - HD 720p.mov":[function(require,module,exports) {
module.exports = "/A thousand word speller - HD 720p.685602ad.mov";
...
var _AThousandWordSpellerHD720p = _interopRequireDefault(require("../assets/A thousand word speller - HD 720p.mov"));
var _gsap = _interopRequireDefault(require("gsap"));
var _ScrollTrigger = require("gsap/ScrollTrigger");
var _ScrollToPlugin = require("gsap/ScrollToPlugin");
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
console.log(_AThousandWordSpellerHD720p.default);

// https://gsap.com/community/forums/topic/32322-clip-path-to-gsap-animation/
// https://developer.mozilla.org/en-US/docs/Web/CSS/clip-path

var scrollActing = false;
// An option to avoid using this is to simply add an event listener for hover
// When I'm hovering/focusing the item, I have the event listener for scroll
// Otherwise, I don't have it

var sidebar = document.getElementById("sidebar");
var currElement = document.querySelector("#first-element");
currElement.classList.add("active");

// <video src="assets/A thousand word speller - HD 720p.mov" muted autoplay loop></video>

var video_address = {
  "A thousand-word speller": _AThousandWordSpellerHD720p.default
};
function createVideo(address) {
  var videoElement = document.createElement("video");

  // Set attributes for the video element
  videoElement.src = address;
  videoElement.muted = true;
  videoElement.autoplay = true;
  videoElement.loop = true;
  var videoHolder = document.querySelector(".video-background");
  var existingVideo = videoHolder.querySelector('video');
  if (existingVideo) {
    // If a video element exists, remove it
    _gsap.default.to(existingVideo, {
      filter: 'blur(40px)',
      opacity: 0,
      duration: 0.5,
      onComplete: function onComplete() {
        return existingVideo.remove();
      }
    });
  }
  videoHolder.appendChild(videoElement);
  _gsap.default.from(videoElement, {
    filter: 'blur(40px)',
    opacity: 0,
    duration: 1
  });
...
var contentIDs = {
  "Mynd Music": "mynd-music",
  "Waterfall": "waterfall"
};

// Instead of manually detecting the middle element
// I can also use scrolltrigger to track all elements
// When they reach the middle I animate
// This approach is also fine, I just have to make an animation that affect middleElemetn
function updateCanvasOnScroll() {
  var elements = document.elementsFromPoint(80, Math.floor(window.innerHeight / 2));
  var liCheck = elements.find(function (element) {
    return element.tagName.toLowerCase() === "li";
  });
  if (liCheck != undefined && liCheck != currElement) {
    currElement.classList.remove("active");
    currElement = liCheck;
    currElement.classList.add("active");
    var headerText = currElement.querySelector('h3').textContent;
    if (!scrollActing) {
      _gsap.default.to(".content-holder", {
        duration: 0,
        scrollTo: {
          y: document.getElementById(contentIDs[headerText]),
          positionInViewport: "center"
        },
        ease: "none"
      });
    }
    var lastElement = currElement;
    setTimeout(function (prevElement) {
      if (prevElement == currElement) {
        console.log("Remained");
        if (video_address.hasOwnProperty(headerText)) {
          console.log("Creating video...");
          createVideo(video_address[headerText]);
        } else {
          deleteVideo();
        }
      }
...
var listParent = document.querySelector("#sidebar");
var listChilds = Array.from(listParent.children);
function checkNextElement() {
  var targets = document.querySelectorAll('.portfolio-content');
  var currElementIndx = listChilds.indexOf(currElement);
  var currElementPos = _ScrollTrigger.ScrollTrigger.positionInViewport(targets[currElementIndx - 1], "bottom").toFixed(2);
  if (currElementIndx > 1) {
    var prevElementPosBottom = _ScrollTrigger.ScrollTrigger.positionInViewport(targets[currElementIndx - 2], "bottom").toFixed(2);
    if (prevElementPosBottom > 0.6) {
      scrollToProject("previous");
    }
  }
  if (currElementPos < 0.3) {
    scrollToProject("next");
  }
}
function scrollToProject(option) {
  var currElementIndx = listChilds.indexOf(currElement);
  console.log(currElement);
  var nextElement = listChilds[currElementIndx];
  if (option == "previous") {
    nextElement = listChilds[currElementIndx - 2];
  }
  if (nextElement) {
    currElement.classList.remove("active");
    // Create a global variable set to true
    scrollActing = true;
    _gsap.default.to(listParent, {
      duration: 0.7,
      scrollTo: {
        y: nextElement,
        positionInViewport: "center"
...
var sec = document.querySelectorAll(".portfolio-content"); // Will be replaced by ID
var sections = _gsap.default.utils.toArray(sec.children);
sec.forEach(function (sec) {
  return _gsap.default.utils.toArray(sec.children).forEach(function (section) {
    if (section.localName == "video") {
      console.log("Video!");
...
/*
var sec = document.querySelector(".portfolio-content") // Will be replaced by ID
let sections = gsap.utils.toArray(sec.children);
// console.log(sections);
var myScroller = document.querySelector(".content-holder");
...
*/
},{"../assets/A thousand word speller - HD 720p.mov":"assets/A thousand word speller - HD 720p.mov","gsap":"../node_modules/gsap/index.js","gsap/ScrollTrigger":"../node_modules/gsap/ScrollTrigger.js","gsap/ScrollToPlugin":"../node_modules/gsap/ScrollToPlugin.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","scripts/navbar.js"], null)
//# sourceMappingURL=/navbar.f3bbc6a1.js.map